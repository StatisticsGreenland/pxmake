...
make_template(data_df = df %>% mutate(n = NA), 
              languages = c("en"),
              time_variable = "taar",
...
```{r}
micromake2(data_df = df, 
           metadata_path = micro_metadata
           )
```

## continue on micromake2 example. pxmake should be able to handle more columns in metadata than in data table
