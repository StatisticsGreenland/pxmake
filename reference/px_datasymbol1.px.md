...
Inspect or change DATASYMBOL1.

## Usage

``` r
px_datasymbol1(x, value, validate)

# S3 method for class 'px'
px_datasymbol1(x, value, validate = TRUE)
```
...
- value:
  ...
  languages. If 'value' is missing, the current DATASYMBOL1 is returned.
  If NULL, DATASYMBOL1 is removed.
...
``` r
# Set DATASYMBOL1 for all languages
x1 <-
  px(population_gl) |>
  px_datasymbol1('missing')

# Print DATASYMBOL1
px_datasymbol1(x1)
#> [1] "missing"

# Set DATASYMBOL1 for individual languages
library(tibble)
x2 <-
  x1 |>
  px_languages(c('en', 'kl')) |>
  px_datasymbol1(tribble(~language, ~value,
                      'en', 'missing',
                      'kl', 'amigaataapput'))
px_datasymbol1(x2)
#> # A tibble: 2 × 2
