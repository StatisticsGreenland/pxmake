test_that("Total is added to one variable", {
  ...
  expect_equal(
    add_total_level_to_var(input, "cohort") |> dplyr::arrange_all(),
...
test_that("metadata without elimination values use default value", {
  levels <-
    px(
      input = get_metadata_path("BEXSTA"),
  ...
  expect_equal(levels, c("Total", "M", "K"))
})
...
test_that("Add totals work for multiple languages", {
  ...
  expect_identical(gender_levels, c("Total", "female", "male"))
})
