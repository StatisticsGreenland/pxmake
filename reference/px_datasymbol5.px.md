...
Inspect or change DATASYMBOL5.

## Usage

``` r
px_datasymbol5(x, value, validate)

# S3 method for class 'px'
px_datasymbol5(x, value, validate = TRUE)
```
...
- value:
  ...
  languages. If 'value' is missing, the current DATASYMBOL5 is returned.
  If NULL, DATASYMBOL5 is removed.
...
``` r
# Set DATASYMBOL5 for all languages
x1 <-
  px(population_gl) |>
  px_datasymbol5('missing')

# Print DATASYMBOL5
px_datasymbol5(x1)
#> [1] "missing"

# Set DATASYMBOL5 for individual languages
library(tibble)
x2 <-
  x1 |>
  px_languages(c('en', 'kl')) |>
  px_datasymbol5(tribble(~language, ~value,
                      'en', 'missing',
                      'kl', 'amigaataapput'))
px_datasymbol5(x2)
#> # A tibble: 2 × 2
