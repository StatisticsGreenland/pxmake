...
writepx.gs <- function ( obj.px, filename, heading = NULL, stub = NULL,
                       ...
                       fileEncoding = "ISO-8859-1" )
{
  ...
  obj.px[['LAST-UPDATED']]$value <- format(Sys.time(), "%Y%m%d %H:%M")
  obj.px[['CHARSET']]$value      <- ifelse(fileEncoding == "ISO-8859-1", 'ANSI', fileEncoding)
  obj.px$INFO$value              <- "Modified pxR write function"
  ...
  names(obj.px) <- gsub("\\-da-", "[da]", names(obj.px))
  names(obj.px) <- gsub("\\-kl-", "[kl]", names(obj.px))
  names(obj.px) <- gsub("\\-fo-", "[fo]", names(obj.px))
  names(obj.px) <- gsub("\\-fi-", "[fi]", names(obj.px))
  names(obj.px) <- gsub("\\[en]", "", names(obj.px))
  ...
  for (key in new.order ) {
    ...
    if ( names(obj.px[[key]])[1] == 'value' ) {  
      wf( key, "=")
      wf( paste( requote(gsub("\\."," ",obj.px[[key]]$value)), collapse = ',') )
      wf(';\n')
      next
    } 
    ...
    for (subkey in names(obj.px[[key]])){
      #
      # LARP / change code to replace . in value names with blanks
      #
      # old
      # wf( key, '("', subkey, '")=' )
      # new
