...
Inspect or change DATASYMBOL6.

## Usage

``` r
px_datasymbol6(x, value, validate)

# S3 method for class 'px'
px_datasymbol6(x, value, validate = TRUE)
```
...
- value:
  ...
  languages. If 'value' is missing, the current DATASYMBOL6 is returned.
  If NULL, DATASYMBOL6 is removed.
...
``` r
# Set DATASYMBOL6 for all languages
x1 <-
  px(population_gl) |>
  px_datasymbol6('missing')

# Print DATASYMBOL6
px_datasymbol6(x1)
#> [1] "missing"

# Set DATASYMBOL6 for individual languages
library(tibble)
x2 <-
  x1 |>
  px_languages(c('en', 'kl')) |>
  px_datasymbol6(tribble(~language, ~value,
                      'en', 'missing',
                      'kl', 'amigaataapput'))
px_datasymbol6(x2)
#> # A tibble: 2 × 2
