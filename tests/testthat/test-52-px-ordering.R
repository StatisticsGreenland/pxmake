...
test_that("data columns are stored as ordered factors matching cells1 order", {
  x <- px(population_gl)

  non_figures <- setdiff(names(x$data), px_figures(x))

  for (col in non_figures) {
    expect_true(is.ordered(x$data[[col]]))
  }
