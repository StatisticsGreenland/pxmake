...
Inspect or change DATASYMBOL4.

## Usage

``` r
px_datasymbol4(x, value, validate)

# S3 method for class 'px'
px_datasymbol4(x, value, validate = TRUE)
```
...
- value:
  ...
  languages. If 'value' is missing, the current DATASYMBOL4 is returned.
  If NULL, DATASYMBOL4 is removed.
...
``` r
# Set DATASYMBOL4 for all languages
x1 <-
  px(population_gl) |>
  px_datasymbol4('missing')

# Print DATASYMBOL4
px_datasymbol4(x1)
#> [1] "missing"

# Set DATASYMBOL4 for individual languages
library(tibble)
x2 <-
  x1 |>
  px_languages(c('en', 'kl')) |>
  px_datasymbol4(tribble(~language, ~value,
                      'en', 'missing',
                      'kl', 'amigaataapput'))
px_datasymbol4(x2)
#> # A tibble: 2 × 2
