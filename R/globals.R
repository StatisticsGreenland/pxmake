...
get_excel_metadata_path <- function(table_name) {
  file.path(metadata_dir, stringr::str_glue("metadata_{stringr::str_to_upper(table_name)}.xlsx"))
}

get_source_data_path <- function(table_name) {
  file.path(data_raw_dir, stringr::str_glue("{stringr::str_to_upper(table_name)}.rds"))
}

get_px_file_path <- function(table_name) {
  file.path(data_dir, stringr::str_glue("{stringr::str_to_upper(table_name)}.px"))
}
