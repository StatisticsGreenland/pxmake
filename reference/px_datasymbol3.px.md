...
Inspect or change DATASYMBOL3.

## Usage

``` r
px_datasymbol3(x, value, validate)

# S3 method for class 'px'
px_datasymbol3(x, value, validate = TRUE)
```
...
- value:
  ...
  languages. If 'value' is missing, the current DATASYMBOL3 is returned.
  If NULL, DATASYMBOL3 is removed.
...
``` r
# Set DATASYMBOL3 for all languages
x1 <-
  px(population_gl) |>
  px_datasymbol3('missing')

# Print DATASYMBOL3
px_datasymbol3(x1)
#> [1] "missing"

# Set DATASYMBOL3 for individual languages
library(tibble)
x2 <-
  x1 |>
  px_languages(c('en', 'kl')) |>
  px_datasymbol3(tribble(~language, ~value,
                      'en', 'missing',
                      'kl', 'amigaataapput'))
px_datasymbol3(x2)
#> # A tibble: 2 × 2
